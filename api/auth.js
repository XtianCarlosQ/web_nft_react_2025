const { signToken, setAuthCookie, clearAuthCookie, verifyTokenFromCookie } = require("./_lib/auth");
  ...
  try {
    ...
    if (pathname.endsWith("/api/auth/login") && req.method === "POST") {
      ...
      if (!process.env.ADMIN_USERNAME || !process.env.ADMIN_PASSWORD) {
        res.statusCode = 500;
        return res.end(JSON.stringify({ ok: false, error: "missing_env_admin_credentials" }));
      }
      if (!process.env.JWT_SECRET) {
        res.statusCode = 500;
        return res.end(JSON.stringify({ ok: false, error: "missing_env_jwt_secret" }));
      }
    ...
    if (pathname.endsWith("/api/auth/me") && req.method === "GET") {
      ...
      return res.end(JSON.stringify({ authenticated: true, user: { name: user.u } }));
    }

    // /api/auth/logout -> POST or GET
    if (pathname.endsWith("/api/auth/logout") && (req.method === "POST" || req.method === "GET")) {
      clearAuthCookie(res);
      return res.end(JSON.stringify({ ok: true }));
    }
