...
export default function ServiceFormModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      if (service) {
        // For existing services, preserve order or set to empty string if archived
        setData({
          ...service,
  ...
  function submit(e) {
    ...
    // Validación: ES obligatorio para todos los campos clave
    ...
    if (hasMissing) {
      ...
      // Forzar pestaña ES para mostrar los campos requeridos
      setActiveLang("es");
      // Show visible tooltips for missing fields
      const tooltips = {};
    ...
    // Autorellenar EN a partir de ES si faltan campos
  ...
  // Helper: adapt admin service shape to ServiceCard props
  ...
  // When viewing, show only the card (DRY)
  if (mode === "view") {
    return (
      <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Layout espejo al card: formulario a la izquierda, card a la derecha */}
            ...
            <div className="pt-2 flex gap-2">
              ...
              {mode === "edit" && data.archived && (
                <button
                  ...
                  onClick={() => {
                    // Validar orden antes de restaurar
                    const invalid = data.order === "" || Number(data.order) < 1;
                    if (invalid) {
                      setSubmitAttempted(true);
                      // Show visible tooltip for order field
                      setVisibleTooltips({ order: true });
                      setTimeout(() => setVisibleTooltips({}), 1000);
                      return;
                    }
                    // Restaurar usando el orden provisto
...
function IconPicker({ onSelect, onClose }) {
  const [tab, setTab] = useState("local"); // local | web
  ...
  useEffect(() => {
    ...
    async function search() {
      ...
      try {
        ...
        // Return full names like "lucide:database"
...
function ServicePreviewCard({ service, lang }) {
  const content = service?.[lang] || {};
  const phone = service?.whatsapp || "";
  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      `Buen día, estoy interesado en el servicio de ${
        content.title || "sus servicios"
      }`
    );
    if (phone) window.open(`https://wa.me/${phone}?text=${message}`, "_blank");
  };
  return (
    <div className="rounded-2xl border border-gray-200 shadow-md bg-white w-full max-w-sm">
        ...
        >
          <div className="text-red-600">
            <div className="w-16 h-16 flex items-center justify-center">
              <RenderIcon iconName={service.icon} className="w-10 h-10" />
            </div>
          </div>
        </div>
        <div className="flex flex-col flex-1 px-3 py-2">
          <div>
            <h3 className="text-[16px] font-bold text-gray-900 leading-tight line-clamp-2 mb-2">
              {content.title || "Título del Servicio"}
            </h3>
            <p className="text-sm text-gray-600 leading-snug line-clamp-3 mb-3">
              {content.description || "Descripción del servicio"}
            </p>
          </div>
          <ul className="space-y-1 text-sm text-gray-700 max-h-28 overflow-hidden">
            {(content.features || [""]).filter(Boolean).length ? (
              content.features.map((feature, i) => (
                <li key={i} className="flex items-start">
                  <span className="w-1.5 h-1.5 bg-red-600 rounded-full mr-2 mt-1.5" />
          ...
          <button
            className="mt-3 w-full btn-cta py-2 px-4 text-sm font-medium gap-2"
