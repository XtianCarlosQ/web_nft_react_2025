...
export default function TeamFormModal({
  ...
  async function uploadImage(file) {
    // Lee archivo como base64 y envía a mock API para guardar en /public/uploads/team/
  ...
  async function handleFile(file) {
    try {
      ...
      // fallback preview local si falla
      const objUrl = URL.createObjectURL(file);
      setData((d) => ({ ...d, photo: objUrl }));
      setUploadMsg(
        "No se pudo subir la imagen. Se usará una vista previa local."
      );
    }
  }
        ...
        >
          ...
          {!isView && (
            <form className="space-y-3" onSubmit={submit}>
              ...
              {/* 3) Foto con drag & drop + botón buscar */}
              <div>
          ...
          {/* Preview pane (col 2) with tabs for overlay/plain */}
